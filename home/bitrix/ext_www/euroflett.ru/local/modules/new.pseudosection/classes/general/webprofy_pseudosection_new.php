...
/**
 * Class WPPseudosection
 */
class WPPseudosectionNew
{

	private static $table = 'new_webprofy_pseudosection';
	private static $MLANG = 'WP_PSEUDOSECTION_NEW_';
	 ...
	 * @return bool|array
	 */
	public static function getClosestNotPseudoSectionAdmin($sectionId, $arFilter, $iblockID){
		...
		if($obInside = $resInside->GetNext()){
			...
			if(count($sectionValue) > 0 && $obInside['IBLOCK_SECTION_ID'] > 0){
				if($sectionValue['is_filter'] == 'Y'){
					...
					$functionReturn = self::getClosestNotPseudoSectionAdmin($sectionId, $arFilter, $iblockID);
					if($functionReturn){
	...
	function OnBuildGlobalMenuHandler(&$aGlobalMenu, &$aModuleMenu){
		...
		$arMenu = array(
			...
			"url"  => "webprofy_pseudosection_new.php?lang=".LANGUAGE_ID,
