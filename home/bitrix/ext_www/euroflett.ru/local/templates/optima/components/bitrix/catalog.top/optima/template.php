...
if($_REQUEST['AJAX_ITEM_APPEND'] == 'Y') {
...
<?if (!empty($arResult['ITEMS'])) {?>
	<section class="catalog-top catalog-popular-items catalog-similar-items" id="catalog-top-<?=$component_counter?>">
		<?if(!$arParams["NO_SHOW_TITLE"]):?> <p class="popular-h2"><?= $title ?><?= $linkToAll ?></p><? else: ?><br><?endif;?>
		<? if (!empty($arResult['ITEMS'])): ?>
		<div class="catalog-items">
			<?
			//shuffle($arResult['ITEMS']);
			//$items = array_slice($arResult['ITEMS'], 0, 9, true);
			...
			foreach ($items as $key => $arItem){
                ...
                $tmp_url = parse_url($arItem['~BUY_URL']);
                parse_str($tmp_url['query'], $tmp_url_params);
                unset($tmp_url_params['bxajaxid'], $tmp_url_params['AJAX_ITEM_APPEND'], $tmp_url_params['test']);
                $arItem['~BUY_URL'] = $tmp_url['path'] . '?' . http_build_query($tmp_url_params);
				...
				<div class="catalog-item" id="<?=$strMainID;?>">
					<a href="<?=$arItem['DETAIL_PAGE_URL'];?>" class="catalog-item-image">
                        ...
                        $requiredPropertiesCodes = array('_FREE_DELIVERY', '_FREE_SETUP', 'NEW', 'HIT', 'SPECIAL_OFFER', 'STATUS_NALICHIJA', '_FREE_STORAGE','OUT_OF_PRODUCTION');
                    ...
                    <!-- </ul> -->
						...
						<!-- <img itemprop="image" src="<?=$arItem['PREVIEW_PICTURE']['SRC'];?>" alt="<?=$imgTitle;?>" title="<?=$imgTitle;?>" /> -->
						<img itemprop="image" src="<?=$arItem['MIN_IMG'];?>" alt="<?=$imgTitle;?>" title="<?=$imgTitle;?>" />
						<?if ($arItem['LABEL']){?>
					...
					<div class="product-labels">
						<?
							if ($arItem['PROPERTIES']['_FREE_DELIVERY']['VALUE_XML_ID'] === 'Y') {
                            echo '
                                <a class="product-label-link" href="/payment-and-delivery/" rel="nofollow"><div class="product-label product-label--delivery">
                            ...
                            if ($arItem['PROPERTIES']['_FREE_STORAGE']['VALUE_XML_ID'] === 'Y') {
                            echo '
					...
					<div class="catalog-item-title-and-description">
						...
						<div class="catalog-item-title"><a itemprop="name" href="<?=$arItem['DETAIL_PAGE_URL'];?>"><?=$productTitle;?></a></div>
					</div>
                    <div class="catalog-item-price-and-buy">
                        <div class="catalog-item-art">
                            <strong>Артикул</strong>
                            <span class="catalog-articul"><?= $arItem['PROPERTIES']['ARTNUMBER']['VALUE'] ?></span>
                        </div>
                        ...
                        <?if ($arItem['PROPERTIES']['OUT_OF_PRODUCTION']['VALUE_XML_ID'] === 'Y'): ?>
                        ...
                        <span itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                            ...
                            {
                                ?>
                                <div class="old_price">старая цена:&nbsp;&nbsp;<div><?=number_format($arItem['PROPERTIES']['OLDPRICE']['VALUE'], 0, " ", " ");?> руб.</div></div>
                                <?php
                            }
                            ?>
							<div class="catalog-item-price">цена:<span class="price" itemprop="price" content="<?=str_replace(" ", "", $arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE']); ?>">
							...
							<div class="catalog-item-buy with-compare">
								<? if ($canBuy): ?>
                                    ...
                                    <? if ($arItem['CATALOG_QUANTITY']>0) echo '<div class="is-available"><link itemprop="availability" href="http://schema.org/InStock"/>'.(!empty($arItem['PROPERTIES']['STATUS_NALICHIJA']['VALUE']) ? $arItem['PROPERTIES']['STATUS_NALICHIJA']['VALUE'] : 'В наличии').'</div>';
                                    else echo '<link itemprop="availability" href="http://schema.org/InStock"/><div class="is-not-available"'.(!empty($arItem['PROPERTIES']['STATUS_NALICHIJA']['VALUE']) ? '' : ' data-availability="not-availability"').'>'.(!empty($arItem['PROPERTIES']['STATUS_NALICHIJA']['VALUE']) ? $arItem['PROPERTIES']['STATUS_NALICHIJA']['VALUE'] : '').'</div>';
                                    // getIsAvailableText($arItem['CATALOG_QUANTITY'] > 0); ?>
                                <? else: ?>
								<div class="catalog-item-buy__box"> 
                                   <a href="#learn-about-admission-<?=$arItem['ID']?>" class="button-buy dark-blue learn-about-admission-button" ><span>Узнать о поступлении</span></a>
								</div>
<div class="form-pupop catalog_top_admission" id="learn-about-admission-<?=$arItem['ID']?>" style="display:none;">
    ...
    <form name="quick_buy" action="/learn_about_admission.php" method="POST">
        ...
        <input type="hidden" name="product_url" value="<?= $arItem['DETAIL_PAGE_URL'] ?>">
		<?
        ...
        <div class="current-position">
            ...
            <h4><?= $arItem['NAME'] ?></h4>
            <strong>
                <span itemprop="price" content="<?=str_replace(" ", "", $arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE']); ?>">
        ...
        <div class="form-questions">
            ...
            <div class="field">
                <div class="right">
                    <input type="text" required id="phone-<?=$arItem['ID']?>" placeholder="Телефон" name="phone">
                </div>
            </div>
			<div class="field">
                <div class="right">
                    <input type="text" required id="email-<?=$arItem['ID']?>" placeholder="E-mail" name="email">
                </div>
            </div>

            <div class="field">
                <div class="right">
                    <input type="checkbox" required id="checkbox1-<?=$arItem['ID']?>" name="agreement" value="true">
                    <label for="checkbox1-<?=$arItem['ID']?>">
                    ...
                    <input type="submit" name="quick_buy" value="Заказать" class="button-primary button-popup" style="opacity: 0.5;">
						...
						<? else:?>
                            <? if ($arItem['CATALOG_QUANTITY'] > 0) { ?>
                                <div class="catalog-item-price">
									<a href="#preorder" class="in-stock-ask-for-price" data-action="preorder" data-item-title="<?= $productTitle ?>" data-item-url="<?=$arItem['DETAIL_PAGE_URL'];?>" data-form-name="Уточнить цену" data-form-submit="Запросить">Есть в наличии.<br />Цену уточняйте у консультантов</a>
								</div>
                            <? } else {
                                ?>
                                <div class="catalog-item-price">
									<a href="#preorder" class="not-in-stock-ask-for-price" data-action="preorder" data-item-title="<?= $productTitle ?>" data-item-url="<?=$arItem['DETAIL_PAGE_URL'];?>">Товар под заказ.<br />Цену уточняйте у консультантов</a>
								</div>
                            <? } ?>
							<?if ($arParams['DISPLAY_COMPARE']){?>
                            ...
                            <?}else{?>
                                <div class="with-compare"><div class="compare_check_box"><div><span></span></div></div></div>
                            <?}?>
                        <? endif ?>
                            <? endif ?>
                    </span>
