...
$element_info = CIBlockElement::GetList(
    array(),
    array(
        'IBLOCK_ID' => $arParams['IBLOCK_ID'], 
...
if(file_exists($_SERVER["DOCUMENT_ROOT"] . '/include/createCatalogMeta.php') && $element_info['ID'] > 0) {
    ...
    if(class_exists('createCatalogMeta')) {
        ...
        $element_meta = $meta_creator -> get_element_meta($arParams['IBLOCK_ID'], $element_info['ID']);
		
        if (isset($seo_title)&& $seo_title) {
			$set_title = 'N';
			$APPLICATION->SetPageProperty("title", $seo_title);
		}
        elseif($element_meta['title'] != '') {            
            $set_title = 'N';
            $APPLICATION->SetPageProperty("title", $element_meta['title']);
        }
    }
}*/

//if (isset($seo_description)&& $seo_description) $APPLICATION->SetPageProperty("description", $seo_description);

/*
if (isset($arResult["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"]) && $arResult["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"]){
		 $set_title = 'N';
         $APPLICATION->SetPageProperty("title", $arResult["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"]);
		}*/

			$update = class_exists('CKokoc') && method_exists('CKokoc', 'GetElementUpdateTime') ? CKokoc::GetElementUpdateTime($arParams['IBLOCK_ID'], $arResult['VARIABLES']) : '';
		...
		if ($ElementID > 0)
		{
			...
			while($ob = $res->GetNextElement())
			{
				...
				if (!empty($arProps['VITRINA']['VALUE']) && $arProps['VITRINA']['VALUE'] == "Да")
					$APPLICATION->SetPageProperty("noindex", '<meta name="robots" content="noindex, nofollow"/>');
			}
			...
			*/?>
			...
			<?
				...
				$arFilterSimilar['CATALOG_AVAILABLE'] = 'Y';
