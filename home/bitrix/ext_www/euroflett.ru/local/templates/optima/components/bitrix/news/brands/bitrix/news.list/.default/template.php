...
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	<div class="news-item<?= ($withImage ? ' with-image' : '') ?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
		<?if($arParams["DISPLAY_PICTURE"]!="N" && is_array($arItem["PREVIEW_PICTURE"])):?>
			...
			<?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
				<a href="<?=$arItem["DETAIL_PAGE_URL"]?>">
					<img src="<?= resizeImageGetSrc($arItem["PREVIEW_PICTURE"]["ID"],"BRANDS_LIST") ?>" alt="<?=$arItem["PREVIEW_PICTURE"]["ALT"]?>"
		...
		<?endif?><a class="name_bb" href="<?=$arItem["DETAIL_PAGE_URL"]?>"><?=$arItem["NAME"]?></a>
