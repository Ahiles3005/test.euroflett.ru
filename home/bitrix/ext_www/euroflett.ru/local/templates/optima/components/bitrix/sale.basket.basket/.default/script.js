	...
	$(document).on('click', '[data-action=delete]', function(e){
		...
		dataLayer.push({
			"ecommerce": {
				"remove": {
					"products": [
						del_products[$this.attr('product-id')]
					]
				}
			}
		});
		//console.log(del_products[$this.attr('product-id')]);
	});
...
function updateBasketList(result){
	...
	var curElem;
	var allItems = [];
	result['BASKET_DATA']['allPreorderSum'] = 0;
	var itemsCnt = 0;
	var itemsPreorderCnt = 0;
	...
	for (i = 0; i < items.length; i++)
	{
		...
		curElem.find('input[name*="QUANTITY"]').val(items[i]['QUANTITY']);
		...
		//if (mb_strpos(mb_strtolower($arItem['NAME']), 'miele') !== false) {}
		if(items[i]['NAME'].toLowerCase().indexOf('miele') + 1) {
			result['BASKET_DATA']['allSum'] -= items[i]['PRICE']*items[i]['QUANTITY'];
			result['BASKET_DATA']['allPreorderSum'] += items[i]['PRICE']*items[i]['QUANTITY'];
			itemsPreorderCnt++;
		}else{
			itemsCnt += parseInt(items[i]['QUANTITY']);
		}
	}
	if (result['BASKET_DATA']['allPreorderSum']>0) {
		result['BASKET_DATA']['allSum_FORMATED'] = numberWithSpaces(result['BASKET_DATA']['allSum']);
		result['BASKET_DATA']['allPreorderSum_FORMATED'] = numberWithSpaces(result['BASKET_DATA']['allPreorderSum']);
	}
	//console.log(result);
	var itemsWrapper = $('.AnDelCanBuy');
	itemsWrapper.parents('#basket_form').find('a[data-list=AnDelCanBuy] span').text(' ('+ itemsCnt +')');

	var itemsPlural = plural(itemsCnt, ['товар', 'товара', 'товаров']);
	itemsWrapper.find('[data-selector=total-text]').text(itemsCnt + ' ' + itemsPlural);
	itemsWrapper.find('[data-selector=total-sum]').html(result['BASKET_DATA']['allSum_FORMATED'] + ' <span class="rub">руб.</span>');

	if (result['BASKET_DATA']['allPreorderSum']>0) {
		var itemsPreorderPlural = plural(itemsPreorderCnt, ['товар', 'товара', 'товаров']);			
		itemsWrapper.find('[data-selector=total-text-preorder]').text(itemsPreorderCnt + ' ' + itemsPreorderPlural);
		itemsWrapper.find('[data-selector=total-sum-preorder]').html(result['BASKET_DATA']['allPreorderSum_FORMATED'] + ' <span class="rub">руб.</span>');
	}
