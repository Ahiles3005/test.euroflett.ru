...
global $searchFilter;
if (isset($searchFilter['=ID']) && count($searchFilter['=ID']) > 0) {
	$resultItems = $searchFilter['=ID'];
	foreach($arResult["ITEMS"] as $arItem) {
		$resultItems[$arItem['ID']] = $arItem;
	}
 ...
 foreach($arResult["ITEMS"] as $key=>$arItem)
  {
   $renderImage = CFile::ResizeImageGet($arItem['PREVIEW_PICTURE']['ID'], Array("width" =>"240", "height" =>"201",BX_RESIZE_IMAGE_PROPORTIONAL, false, false, false, 60 ));
	...
	$db_props = CIBlockElement::GetProperty($arItem["IBLOCK_ID"],$arItem["ID"], array("sort" => "asc"), Array("CODE"=>"ARTNUMBER"));
	if($ar_props = $db_props->Fetch()) {
		$arResult["ITEMS"][$key]['PROPERTIES']['ARTNUMBER']['VALUE'] = $ar_props['VALUE'];
	}
  }

  
foreach($arResult['ITEMS'] as &$arItem) {
    $old_price = CIBlockElement::GetProperty(
        $arItem['IBLOCK_ID'],
        $arItem['ID'],
        [],
        [
    ...
    $arItem['PROPERTIES']['OLDPRICE'] = $old_price;
}
