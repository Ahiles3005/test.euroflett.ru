...
foreach ($arResult["SECTIONS"] as $id => $arSection) {
	$UF = $GLOBALS["USER_FIELD_MANAGER"]->GetUserFields("IBLOCK_".$arSection["IBLOCK_ID"]."_SECTION", $arSection["ID"]);
	if ($UF["UF_BRAND"]["VALUE"]) {
		unset($arResult["SECTIONS"][$id]);
		$arResult["SECTIONS_NO_BRANDS"][$id] = $arSection;
	}
}
$APPLICATION->SetPageProperty("title", "строка");
?>