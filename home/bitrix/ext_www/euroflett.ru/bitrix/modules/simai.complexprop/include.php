...
$GLOBALS["err_m"] = false;
/*
switch(CModule::IncludeModuleEx("simai.complexprop"))
{
	case MODULE_NOT_FOUND: $GLOBALS["err_m"] = GetMessage("SIMAI_MODULE_NOT_FOUND"); $cont = false; break;
	case MODULE_DEMO: $GLOBALS["err_m"] = GetMessage("SIMAI_MODULE_DEMO"); break;
	case MODULE_DEMO_EXPIRED: $GLOBALS["err_m"] = GetMessage("SIMAI_MODULE_DEMO_EXPIRED"); $cont = false; break;
}
*/
				...
				foreach($PROP as $prop_code_ => $prop_fields_)
				{
					if ($prop_fields_["ID"] == $sprop && $prop_fields_["MULTIPLE"] != "Y" && !in_array($prop_fields_["USER_TYPE"], $GLOBALS["SIMAI_COMPLEXPROP_FORBIDDEN_UT"]))
					{
						...
						if ($fl)
						{
							...
							if (IntVal($prop_fields__["VALUE"][$pval_id]))
							{
								echo '<input type="hidden" name="'.$strHTMLControlName["VALUE"].'[FL_VID]['.IntVal($sprop).']" value="'.htmlspecialcharsbx($pval_id).'">';
								echo '<input type="hidden" name="'.$strHTMLControlName["VALUE"].'[FL_ID]['.IntVal($sprop).']" value="'.IntVal($prop_fields__["VALUE"][$pval_id]).'">';
							}							
						}
						echo '</td></tr>';
					}
						...
						if ($prop_fields_["ID"] == $sprop && $prop_fields_["MULTIPLE"] == "Y" && !in_array($prop_fields_["USER_TYPE"], $GLOBALS["SIMAI_COMPLEXPROP_FORBIDDEN_UT"]))
						{
							...
							if ($fl)
							{
								...
								if (IntVal($prop_fields__["VALUE"][$pval_id]))
								{
									...
									echo '<input type="hidden" name="'.$strHTMLControlName["VALUE"].'['.$value_id.'][VALUE][FL_ID]['.IntVal($sprop).']" value="'.IntVal($prop_fields__["VALUE"][$pval_id]).'">';
								}
							}
							echo '</td></tr>';
						}
						...
						{							
							if ($subprop_id > 0)
							{
								...
								if ($prop_value['VALUE']['DEL'] != "y")
								{
									...
									$fl_del = "";
									$fl_arr = Array();
									if (isset($arF["PROPERTY_VALUES"][$subprop_id][$subprop_vid]["VALUE"]))
									{
										...
										if ($fl_id && $fl_name == "" && $fl_del != "y" && $fl_del != "Y")
										{
										...
										{
											if ($subprop_val["VALUE"])
												$GLOBALS["SCP_PVDesc"][$subprop_id][$subprop_val["VALUE"]][$pdesc] = $pdesc;
										}
										elseif ($subprop_val)
											$GLOBALS["SCP_PVDesc"][$subprop_id][$subprop_val][$pdesc] = $pdesc;
									}
